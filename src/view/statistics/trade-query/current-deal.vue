    ...
    common-content(@splitChange="changeTableHeight" :splitValue="splitValue")
        .queryContainer
            .tableContainer(:style="styleObj")
                //- Table(size="small" highlight-row stripe  :columns="holdTableColumns" :data="pageData" :height="tableHeight" @on-sort-change="holdColumnsSort" :loading="tableLoading")
            ...
            .formContainer(:style="formStyle")
                Form(ref="bookForm" :model="bookForm" inline :label-width="80" label-position="left")
                    ...
                    FormItem(label="证券代码" prop="sss")
                        tips-input
                    FormItem(label="交易员ID" prop="xxx")
                        Select(v-model="bookForm.xxx" placeholder="请选择交易员ID")
                            Option(v-for="item in selectArr" :value="item.value" :key="item.value") {{item.label}}
                    FormItem(label="汇总方式" prop="xxx")
                        Select(v-model="bookForm.xxx" placeholder="请选择汇总方式")
                            Option(v-for="item in selectArr" :value="item.value" :key="item.value") {{item.label}}
                .operateBtnView
